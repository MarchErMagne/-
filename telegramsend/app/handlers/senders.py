...
from app.database.models import User, Sender, SenderType, SubscriptionStatus  # ДОБАВЛЕН SubscriptionStatus
...
async def back_to_senders_menu(callback: types.CallbackQuery, state: FSMContext):
    ...
    async for db in get_db():
        ...
        if user.subscription_status != SubscriptionStatus.ACTIVE:  # Теперь импорт есть!
