...
@handle_errors
async def contacts_upload_start(callback: types.CallbackQuery, state: FSMContext):
    ...
    async for db in get_db():
        ...
        if not user or user.subscription_status != SubscriptionStatus.ACTIVE:
...
@handle_errors
async def contacts_add_manual(callback: types.CallbackQuery, state: FSMContext):
    """Добавление контакта вручную"""
    await callback.message.edit_text(
        "👤 <b>Добавление контакта вручную</b>\n\n"
    ...
    await state.set_state(ContactStates.waiting_for_manual_contact)
    await callback.answer()

@router.message(ContactStates.waiting_for_manual_contact)
@handle_errors
async def process_manual_contact(message: types.Message, state: FSMContext):
    """Обработка ручного ввода контакта"""
    contact_input = message.text.strip()
    
    # Определяем тип контакта
    contact_type = None
    if "@" in contact_input and "." in contact_input:
        contact_type = SenderType.EMAIL
    elif contact_input.startswith("@") or contact_input.isdigit():
        contact_type = SenderType.TELEGRAM
    elif contact_input.startswith("+"):
        # Спрашиваем для какой платформы
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [
                    types.InlineKeyboardButton(text="💬 WhatsApp", callback_data="manual_phone_whatsapp"),
                    types.InlineKeyboardButton(text="📞 SMS", callback_data="manual_phone_sms")
                ],
                [types.InlineKeyboardButton(text="🟣 Viber", callback_data="manual_phone_viber")]
            ]
        )
        
        await state.update_data(contact_input=contact_input)
        await message.answer(
            f"📞 <b>Номер: {contact_input}</b>\n\n"
            f"Для какой платформы сохранить этот номер?",
            parse_mode="HTML",
            reply_markup=keyboard
        )
        return
    else:
        await message.answer("❌ Неизвестный формат контакта. Попробуйте еще раз.")
        return
    ...
    async for db in get_db():
        ...
        result = await db.execute(
            select(Contact).where(
                and_(
                    Contact.user_id == user.id,
                    Contact.identifier == contact_input,
                    Contact.type == contact_type
                )
            )
        )
        ...
        if result.scalar_one_or_none():
            await message.answer("⚠️ Такой контакт уже существует")
            await state.clear()
            return
        
        contact = Contact(
            user_id=user.id,
            identifier=contact_input,
            type=contact_type,
            is_active=True
        )
        ...
        type_names = {
            ...
            SenderType.SMS: "SMS",
            SenderType.WHATSAPP: "WhatsApp",
            SenderType.VIBER: "Viber"
        }
        
        await message.answer(
            f"✅ <b>Контакт добавлен!</b>\n\n"
            f"📱 Тип: {type_names[contact_type]}\n"
            f"📝 Контакт: {contact_input}",
            parse_mode="HTML",
...
@router.callback_query(F.data.startswith("manual_phone_"))
@handle_errors
async def process_manual_phone_type(callback: types.CallbackQuery, state: FSMContext):
    """Обработка выбора типа для ручного номера"""
    phone_type = callback.data.split("_")[-1]
    data = await state.get_data()
    ...
    async for db in get_db():
        ...
        if result.scalar_one_or_none():
            await callback.answer("⚠️ Такой контакт уже существует", show_alert=True)
            await state.clear()
            return
        ...
        await callback.message.edit_text(
            f"✅ <b>Контакт добавлен!</b>\n\n"
            f"📱 Тип: {phone_type.upper()}\n"
            f"📞 Номер: {contact_input}",
            parse_mode="HTML",
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [types.InlineKeyboardButton(text="◀️ К контактам", callback_data="contacts_menu")]
...
@handle_errors
async def contacts_lists(callback: types.CallbackQuery):
    ...
    async for db in get_db():
        ...
        if not type_stats:
            await callback.message.edit_text(
                "📭 <b>У вас нет контактов</b>\n\n"
                "Добавьте контакты через загрузку файла или вручную.",
                parse_mode="HTML",
                reply_markup=back_keyboard("contacts_menu")
            )
            return
...
@router.callback_query(F.data.startswith("view_contacts_"))
@handle_errors
async def view_contacts_by_type(callback: types.CallbackQuery):
    """Просмотр контактов по типу"""
    contact_type_str = callback.data.split("_")[2]
    contact_type = SenderType(contact_type_str)
    user_id = callback.from_user.id
    ...
    async for db in get_db():
        ...
        icon = type_icons[contact_type]
        contacts_text = f"{icon} <b>Контакты {contact_type.value.capitalize()}</b>\n\n"
        
        if contacts:
            contacts_text += f"📊 Показано: {len(contacts)} из всех контактов\n\n"
            ...
            for i, contact in enumerate(contacts, 1):
                ...
                contacts_text += f"{i}. {contact.identifier}{name_part}\n"
                
                if i >= 15:  # Ограничиваем вывод
                    remaining = len(contacts) - 15
                    if remaining > 0:
                        contacts_text += f"... и еще {remaining} контактов\n"
                    break
        else:
            contacts_text += "📭 Нет контактов этого типа"
        
        keyboard = types.InlineKeyboardMarkup(
            inline_keyboard=[
                [types.InlineKeyboardButton(text="🗑 Удалить все", callback_data=f"delete_contacts_{contact_type.value}")],
                [types.InlineKeyboardButton(text="◀️ К спискам", callback_data="contacts_lists")]
            ]
        )
...
@router.callback_query(F.data.startswith("delete_contacts_"))
@handle_errors
async def confirm_delete_contacts(callback: types.CallbackQuery):
    ...
    keyboard = types.InlineKeyboardMarkup(
        inline_keyboard=[
            [
                types.InlineKeyboardButton(text="✅ Да, удалить", callback_data=f"confirm_delete_{contact_type.value}"),
                types.InlineKeyboardButton(text="❌ Отмена", callback_data=f"view_contacts_{contact_type.value}")
            ]
        ]
    )
    ...
    await callback.message.edit_text(
        ...
        f"Вы уверены что хотите удалить ВСЕ контакты типа {contact_type.value.capitalize()}?\n\n"
...
@handle_errors
async def delete_contacts_confirmed(callback: types.CallbackQuery):
    """Удаление контактов подтверждено"""
    contact_type_str = callback.data.split("_")[2]
    contact_type = SenderType(contact_type_str)
    user_id = callback.from_user.id
    ...
    async for db in get_db():
        ...
        await callback.message.edit_text(
            ...
            f"Удалено контактов {contact_type.value.capitalize()}: {deleted_count}",
            parse_mode="HTML",
            reply_markup=types.InlineKeyboardMarkup(
                inline_keyboard=[
                    [types.InlineKeyboardButton(text="◀️ К контактам", callback_data="contacts_menu")]
...
@handle_errors
async def contacts_search_start(callback: types.CallbackQuery, state: FSMContext):
            ...
            select(Contact).where(
                and_(
                    ...
                    or_(
                        ...
                        Contact.first_name.contains(query) if Contact.first_name.isnot(None) else False,
                        Contact.last_name.contains(query) if Contact.last_name.isnot(None) else False
                    )
                )
            ).order_by(Contact.created_at.desc()).limit(20)
        )
...
@handle_errors
async def contacts_cleanup_start(callback: types.CallbackQuery):
    """Очистка контактов"""
    user_id = callback.from_user.id
...
@handle_errors
async def cleanup_duplicates(callback: types.CallbackQuery):
    """Удаление дубликатов"""
    user_id = callback.from_user.id
