...
def upgrade() -> None:
    # --- Чистим возможные "хвосты" от прошлых попыток ---
    ...
    op.create_table(
        ...
        # Важно: даём SQLAlchemy самому создать тип (create_type=True по умолчанию)
        sa.Column(
            "subscription_status",
    ...
    op.create_table(
        ...
        sa.Column(
            "type",
            sa.Enum("TELEGRAM", "EMAIL", "WHATSAPP", "SMS", "VIBER", name="sendertype"),
            nullable=False,
        ),
...
def downgrade() -> None:
    ...
    # Чистим типы (если больше не используются)
    op.execute("DROP TYPE IF EXISTS campaignstatus;")
    op.execute("DROP TYPE IF EXISTS sendertype;")
    op.execute("DROP TYPE IF EXISTS subscriptionstatus;")
