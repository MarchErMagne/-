...
def validate_email_address(email: str) -> Tuple[bool, Optional[str]]:
    ...
    if EMAIL_VALIDATOR_AVAILABLE:
        ...
        # Простая проверка с regex если библиотека недоступна
...
def validate_telegram_id(telegram_id: str) -> Tuple[bool, Optional[int]]:
    ...
    try:
        ...
        # ID может быть как положительным (пользователи), так и отрицательным (группы)
        if user_id != 0:
            return True, user_id
        return False, "ID не может быть нулем"
    except ValueError:
...
def validate_telegram_link(link: str) -> Tuple[bool, Optional[str]]:
    ...
    patterns = [
        r'^https://t\.me/[a-zA-Z0-9_]{5,32}$',  # Обычные каналы/группы
        r'^https://t\.me/\+[a-zA-Z0-9_-]+$',    # Пригласительные ссылки
        r'^https://t\.me/joinchat/[a-zA-Z0-9_-]+$',  # Старые пригласительные ссылки
        r'^https://telegram\.me/joinchat/[a-zA-Z0-9_-]+$'  # Альтернативный домен
    ]
    ...
    return False, "Неверный формат Telegram ссылки"

def validate_telegram_contact(contact: str) -> Tuple[bool, Optional[str]]:
    """Универсальная валидация Telegram контакта"""
    contact = contact.strip()
    
    # Проверяем различные форматы
    # 1. Ссылки
    if contact.startswith('https://'):
        return validate_telegram_link(contact)
    ...
    # 4. Username без @
    if contact.replace('_', '').replace('.', '').isalnum():
        is_valid, result = validate_telegram_username(contact)
        if is_valid:
            return True, f"@{result}"
        return is_valid, result
    
    return False, "Неизвестный формат Telegram контакта"
    ...
    for line_num, line in enumerate(lines, 1):
        ...
        for contact in contacts:
            ...
            if contact_type == "email":
                ...
                is_valid, result = validate_telegram_contact(contact)
            else:
